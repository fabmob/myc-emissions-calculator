...
export default function VktCompareBarChart (props: {
    ...
    const vtypes = Object.keys(props.bauVktData)
    ...
    for (let y = 0; y < props.project?.referenceYears?.length || 0; y++) {
        ...
        for (let j = 0; j < vtypes.length; j++) {
            ...
            emissionChartData[emissionChartData.length -1]["BAU - " + vtype] = props.bauVktData[vtype]?.[y] || 0
            emissionChartData[emissionChartData.length -1]["Climate - " + vtype] = props.climateVktData[vtype]?.[y] || 0
            totalForYear += Math.max(props.bauVktData[vtype]?.[y] || 0, props.climateVktData[vtype]?.[y] || 0)
        }
        ...
        <ResponsiveContainer width="90%" height={300}>
            <BarChart margin={{left: 50, top: showPercents? 20: 0}} data={emissionChartData}>
                ...
                {vtypes.map((vtype:string, i:number) => {
                    return [
                        <Bar key={"bau" + i} dataKey={"BAU - " + vtype} fill={`rgba(44, 177, 213, ${1-i/vtypes.length})`} stackId="bau" unit=' MVkt'>
                        ...
                        <Bar key={"climate" + i} dataKey={"Climate - " + vtype} fill={`rgba(162, 33, 124, ${1-i/vtypes.length})`} stackId="climate" unit=' MVkt'>
