    ...
    const nextTrigger = () => {
        ...
        for (let i = 0; i < goalvtypes.length; i++) {
            ...
            for (let j = 0; j < originvtypes.length; j++) {
                ...
                for (let y = 0; y < yearlyValues.length; y++) {
                    ...
                    if (computedASI && computedASI.pkmsEndOfYear[y+1][goalvtype] < 0) {
                        setError(`Error: at least one goal vehicle has a negative pkm at the end of the year (${project.referenceYears[y]}, ${goalvtype}). This can be related to an invalid trip distribution at this step, or invalid inputs at the previous steps (Trip length, avoided or added vkt, changes in occupancy rate)`)
                        return
                    }
            ...
            for (let y = 0; y < totalPerYear.length; y++) {
                ...
                if (total === 0) {
                    setError(`Error: At least one sum of trips parts is zero (${project.referenceYears[y]}, ${goalvtype})`)
                    return
                }
            }
        }
                ...
                >
                    {project.referenceYears && project.referenceYears.slice(1).map((y, yearIndex) => (<Tab eventKey={y} title={y} key={yearIndex}>
                        ...
                        <Table bordered>
                            <thead>
                                <tr>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Goal vehicle</th>
                                    ...
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Pkm {project.referenceYears[yearIndex]} (Mkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Pkm {y} (Mkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Added Pkm to distribute (Mpkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Origin vehicle</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Inventory trip len</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>Src</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Part of trips to shift (%)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 pkm shifted to goal</th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        <Table bordered>
                            <thead>
                                <tr>
                                    ...
                                    {/* <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Vkt {project.referenceYears[yearIndex]} (Mkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Vkt {y} (Mkm)</th> */}
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 tkm {project.referenceYears[yearIndex]} (Mkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 tkm {y} (Mkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Added tkm to distribute (Mtkm)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Origin vehicle</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>Src</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 Part of tkm to shift (%)</th>
                                    <th className="item-sm" style={{whiteSpace: "initial"}}>🛈 tkm shifted to goal</th>
                                </tr>
                            </thead>
                            <tbody>
                                {Object.keys(inputData.vtypes).filter(vtype => isVtypeFreight(vtype)).map((goalvtype) => {
