                    ...
                    {project.referenceYears && project.referenceYears.slice(1).map((y, yearIndex) => (<Tab eventKey={y} title={y} key={yearIndex}><hr></hr>
                        <Table bordered>
                            <colgroup>
                                ...
                                <col className="tablecol4" />{/* Computed VKT */}
                                <col className="tablecol2" />{/* BAU VKT */}
                                        ...
                                        <ItemWithOverlay overlayContent={
                                            <div>Vehicle kilometers travelled. Values for each fuel are computed as
                                                <div style={{backgroundColor: "#C5E8F2", padding: "10px", margin: "10px 0px 10px 0px"}}>
                                                    <Badge className="badge-read-only"><span className="item"><span>Computed VKT after shift (Mkm/y)</span></span></Badge> × <Badge className="badge-read-only"><span className="item"><span>VKT (%)</span></span></Badge> / 100
                                                </div>
                                            </div>
                                ...
                                {Object.keys(inputData.vtypes).map((vtype, index) => {
                                    ...
                                    for (let i = 0; i < ftypes.length; i++) {
                                        ...
                                        fuelJsx.push(<tr key={vtype + ftype}>
                                            ...
                                            <OutputNumberTd value={(parseFloat(value) / 100 * (computedASI?.baseVkt?.[vtype]?.[yearIndex+1] || 0))}></OutputNumberTd>
                                    ...
                                    return [
                                        <tr key={vtype}>
                                            ...
                                            <td>All</td>
                                            <OutputNumberTd value={computedASI?.baseVkt?.[vtype]?.[yearIndex+1] || 0}></OutputNumberTd>
